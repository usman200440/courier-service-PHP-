...
if(isset($_POST['register'])){
    ...
    $name1=$_POST['name'];
    $stt=$_POST['status'];
    $se="SELECT * FROM `message` WHERE track='$trr'";
    $res12=mysqli_query($connect,$se);
$check=mysqli_num_rows($res12);

    if($check>0)
    {
        echo'<script>alert("data already inserted now only update please")</script>';
    }
    else
    {
        $que="INSERT INTO `message`(`id`, `name`, `message`, `track`, `status`) 
        VALUES (null,'$name1','$msg','$trr','$stt')";
